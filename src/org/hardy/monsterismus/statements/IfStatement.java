...
public class IfStatement extends ProgrammingStatement {
  ...
  private class IfStatementInstance extends StatementInstance implements
      Nestable {
    ...
    public void draw(final PGraphics $graphics) {
      ...
      if (_consequent != null) {
        $graphics.text("else", _pos.x + 3, _pos.y + _consequent.getHeight()
            + 35);
    ...
    public void setPos(final float $x, final float $y) {
      ...
      if (_alternative != null)
        if (_consequent != null)
          _alternative
              .setPos(_pos.x + 5, _pos.y + _consequent.getHeight() + 40);
        else
    ...
    public StatementInstance instanceUnder(final float $x, final float $y) {
      if ($x > _pos.x && $y > _pos.y && $x < _pos.x + BASE_WIDTH
          && $y < _pos.y + getHeight()) {
